...
if (_gameId){
  ...
  $(".game-id").val(window.location.href);
...
socket.on('correct letter', (data, playersOnline)=>{
  ...
  setPlayersOnline(playersOnline);
  var playerIndex = determinePlayerIndexFromNameList(_player.id);
  selectKey(data.letter);

  //update Phrase
  $("#phrase-display").text(data.phrase);
  if (_activeGuesser != data.activeGuesser){
    formatNameList(data.activeGuesser);
  }
  _activeGuesser = data.activeGuesser;
...
function selectKey(letter){
  $(`#${letter.toUpperCase()}-key`).addClass("already-selected");
}
...
function deselectAllKeys(letter){
  $(".jkey").each((i, $key)=>{$($key).removeClass("already-selected")});
}
