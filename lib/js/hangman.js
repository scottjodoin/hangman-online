...
socket.on('reset information', (data)=>{
...
socket.on('phrase rejected', (data)=>{
...
socket.on('incorrect letter', (data)=>{
  var playerIndex = determinePlayerIndexFromNameList(_player.id);
...
socket.on("rotate and new round", ()=>{
  rotateNameList(()=>{
    formatNameList(_activeGuesser);
    setupNewRound();
  });
...
document.onkeypress = function(e) {
  ...
  /*//TODO: trigger the key - bubbling issues.
  console.log(letter);
  $key = $( `.jkey letter:contains(${letter})`);
  console.log($key);
  $key.trigger( "click" );
  //*/
  var guessedLetters = $("#guessed-letters-display").text().toUpperCase();
